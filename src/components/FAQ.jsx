...
const FAQ = ({ faqs }) => {
  const [openIndex, setOpenIndex] = useState(0); // desktop: first card open
  const [mobileOpenIndex, setMobileOpenIndex] = useState(-1); // mobile: none open by default
  ...
  return (
    <section className="w-full py-8 md:py-12 bg-[var(--color-bg-white)]">
      <div className="flex flex-col items-center justify-center px-4 sm:px-6">
        ...
        {/* Mobile: accordion — question + arrow, tap to show answer */}
        <div className="w-full max-w-2xl md:hidden space-y-2">
          {faqs.map((faq, idx) => {
            const isOpen = idx === mobileOpenIndex;
            return (
              <div
                key={idx}
                className="rounded-xl border border-gray-200/80 bg-white overflow-hidden shadow-sm"
              >
                <button
                  type="button"
                  onClick={() => setMobileOpenIndex(isOpen ? -1 : idx)}
                  className="w-full flex items-center justify-between gap-3 text-left px-4 py-4 active:bg-gray-50"
                >
                  <span className="font-medium text-sm md:text-base text-[var(--color-text-primary)] pr-2">
                    {faq.question}
                  </span>
                  <span
                    className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center bg-[var(--color-primary-50)] text-[var(--color-primary-600)] transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`}
                  >
                ...
                >
                  <div className="overflow-hidden">
                    <div className="px-4 pb-4 pt-0">
                      <p className="text-sm text-[var(--why-choose-secondary)] leading-relaxed whitespace-pre-line border-t border-gray-100 pt-3">
                        {faq.answer}
                      </p>
                    </div>
                  </div>
        ...
        {/* Desktop: Cards */}
        <div className="w-full hidden md:flex flex-row md:justify-center md:gap-4 md:overflow-x-auto">
          {faqs.map((faq, idx) => {
            const isOpen = idx === openIndex;
            const qNo = String(idx + 1).padStart(2, "0");
            ...
            return (
              <button
                ...
                onClick={() => setOpenIndex(idx)}
                className={[
                  "relative flex-shrink-0 rounded-2xl text-left overflow-hidden",
                  ...
                  "[transition-property:width,background-color] [transition-duration:700ms] [transition-timing-function:linear]",
                ].join(" ")}
                style={{
                  background: isOpen
                    ? "var(--color-primary-600)"
              ...
              >
                ...
                <span
                  className={[
                    "absolute top-4 right-4 md:top-6 md:right-6 w-8 h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center z-20",
                    "[transition-property:background-color] [transition-duration:700ms] [transition-timing-function:linear]",
                    isOpen ? "bg-white/20" : "bg-white",
                  ].join(" ")}
                >
                  {isOpen ? (
                ...
                <div
                  className={[
                    ...
                    "[transition-property:opacity] [transition-duration:650ms] [transition-timing-function:linear]",
                ...
                >
                  ...
                  <h3 className="mt-2 text-lg md:text-2xl font-bold text-white pr-10 md:pr-0">
                    {faq.question}
                  </h3>
                ...
                {/* CLOSED CONTENT (normal flow) */}
                <div
                  className={[
                    "relative z-0 h-full flex items-end",
                    "[transition-property:opacity] [transition-duration:200ms] [transition-timing-function:linear]",
